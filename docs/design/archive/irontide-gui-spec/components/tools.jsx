          ...
          <div style={{flex:1, overflowY:'auto'}}>
            {MOCK.rssItems.filter(it => feed==='Linux ISOs' ? it.feed==='Linux ISOs' : true).map(it => (
              <div key={it.id} style={{
                ...
                <span className="mono" style={{flex:1, fontSize: 12.5}}>{it.title}</span>
                {it.matched ? <IT.Chip tone="complete">matched: {it.matched}</IT.Chip> : null}
                <span className="mono" style={{fontSize: 11, color:'var(--fg-2)', width: 70, textAlign:'right'}}>{it.size}</span>
                <span className="mono" style={{fontSize: 11, color:'var(--fg-3)', width: 120, textAlign:'right'}}>{it.date}</span>
                <IT.Btn variant="solid" size="sm" icon={Icon.download({size:12})}>Download</IT.Btn>
              </div>
            ))}
          </div>
      ...
      <PageShell title="Search" subtitle="Query enabled plugins across trackers and indexers">
        ...
        <div style={{display:'flex', gap: 12}}>
          <div style={{flex:1, background:'var(--bg-1)', border:'1px solid var(--border-1)', borderRadius:'var(--r-md)', overflow:'hidden'}}>
            <div style={{display:'flex', padding:'8px 12px', fontSize: 11, textTransform:'uppercase', color:'var(--fg-2)', borderBottom:'1px solid var(--border-1)', fontWeight: 600, letterSpacing:'.04em'}}>
              <span style={{flex:'1 0 280px'}}>Name</span>
              <span style={{width: 90, textAlign:'right'}}>Size</span>
              <span style={{width: 70, textAlign:'right'}}>Seeds</span>
              <span style={{width: 70, textAlign:'right'}}>Peers</span>
            ...
            {MOCK.searchResults.map((r,i) => (
              <div key={i} style={{display:'flex', padding:'10px 12px', borderBottom:'1px solid var(--divider)', alignItems:'center', fontSize: 12.5}}>
                <span style={{flex:'1 0 280px'}}>{r.name}</span>
                <span className="mono" style={{width: 90, textAlign:'right'}}>{r.size}</span>
                <span className="mono" style={{width: 70, textAlign:'right', color:'var(--st-seeding)'}}>{r.seeds}</span>
                <span className="mono" style={{width: 70, textAlign:'right', color:'var(--fg-2)'}}>{r.peers}</span>
                <span style={{width: 130, color:'var(--fg-2)'}}>{r.engine}</span>
                <span className="mono" style={{width: 100, textAlign:'right', color:'var(--fg-3)', fontSize: 11}}>{r.date}</span>
      ...
      <PageShell title="Create new torrent" subtitle="Bundle local files for sharing">
        <div style={{maxWidth: 760}}>
          ...
          <div style={{display:'flex', gap: 6, marginBottom: 14}}>
            <IT.TextInput mono value="/Volumes/Storage/MyRelease" width="100%"/>
