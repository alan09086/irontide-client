...
(() => {
  ...
  const COLUMNS = [
    ...
    { id:'seeds',   label:'Seeds',    w: 94,  align:'right', mono: true },
    { id:'peers',   label:'Peers',    w: 94,  align:'right', mono: true },
    ...
    return (
      <div style={{flex:1, display:'flex', flexDirection:'column', minHeight: 0, background: 'var(--bg-0)'}}>
          ...
          {rows.map((t, i) => {
            ...
            return (
              <div key={t.id} onClick={() => setSelected(t.id)}
                style={{
                  display:'flex', height: 'var(--row-h)', overflow:'hidden',
              ...
              >
                ...
                {/* Size */}
                <div className="mono" style={{width: 78, padding:'0 10px', textAlign:'right', whiteSpace:'nowrap', display:'flex', alignItems:'center', justifyContent:'flex-end'}}>{t.size}</div>
                {/* Progress */}
                <div style={{width: 130, padding:'0 10px', display:'flex', alignItems:'center'}}>
                  <IT.ProgressBar value={t.progress} tone={t.status} showLabel/>
                </div>
                {/* Status label */}
                <div style={{width: 90, padding:'0 10px', display:'flex', alignItems:'center', textTransform:'capitalize', color:'var(--fg-1)', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis'}}>
                  {t.status}
                </div>
                {/* Seeds */}
                <div className="mono" style={{width: 94, padding:'0 10px', textAlign:'right', whiteSpace:'nowrap', fontSize:11.5, display:'flex', alignItems:'center', justifyContent:'flex-end', color:'var(--fg-1)'}}>{t.seeds}</div>
                <div className="mono" style={{width: 94, padding:'0 10px', textAlign:'right', whiteSpace:'nowrap', fontSize:11.5, display:'flex', alignItems:'center', justifyContent:'flex-end', color:'var(--fg-1)'}}>{t.peers}</div>
                <div className="mono" style={{width: 84, padding:'0 10px', textAlign:'right', whiteSpace:'nowrap', display:'flex', alignItems:'center', justifyContent:'flex-end', color: t.dl!=='0 B/s' ? 'var(--st-downloading)' : 'var(--fg-2)'}}>{t.dl}</div>
                <div className="mono" style={{width: 84, padding:'0 10px', textAlign:'right', whiteSpace:'nowrap', display:'flex', alignItems:'center', justifyContent:'flex-end', color: t.ul!=='0 B/s' ? 'var(--st-seeding)' : 'var(--fg-2)'}}>{t.ul}</div>
                <div className="mono" style={{width: 60, padding:'0 10px', textAlign:'right', whiteSpace:'nowrap', display:'flex', alignItems:'center', justifyContent:'flex-end', color: t.ratio >= 1 ? 'var(--st-seeding)' : 'var(--fg-1)'}}>{t.ratio.toFixed(2)}</div>
                <div className="mono" style={{width: 72, padding:'0 10px', textAlign:'right', whiteSpace:'nowrap', display:'flex', alignItems:'center', justifyContent:'flex-end', color:'var(--fg-2)'}}>{t.eta}</div>
                ...
                <div className="mono" style={{width: 140, padding:'0 10px', display:'flex', alignItems:'center', color:'var(--fg-2)', fontSize: 11.5, whiteSpace:'nowrap', overflow:'hidden'}}>{t.added}</div>
        ...
        <div style={{
          ...
          <span style={{flex:1}}/>
          <span className="mono" style={{whiteSpace:'nowrap'}}>⇣ 13.7 MB/s · ⇡ 3.2 MB/s · 128 peers · DHT: 1,024 nodes</span>
        </div>
      </div>
    );
  }
