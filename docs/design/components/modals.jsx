        ...
        <div style={{display:'grid', gridTemplateColumns:'minmax(0,1fr) minmax(0,1fr)', gap: 14}}>
          <div>
            ...
            <div style={{display:'flex', gap: 6}}>
              <IT.TextInput mono value="/mnt/storage/Torrents/Software" width="100%"/>
          ...
          <div style={{display:'flex', flexDirection:'column', gap: 8}}>
            ...
            <IT.Toggle on={sequential} onChange={setSequential} label="Download sequentially"/>
            ...
            <IT.Toggle on={true} label="Automatic torrent management"/>
          </div>
        </div>
    ...
    return (
      ...
      >
        ...
        <div style={{display:'flex', gap: 6, marginBottom: 14}}>
          <IT.TextInput mono value="/mnt/storage/MyAlbum" width="100%"/>
...
udp://open.stealth.si:80/announce"
        ...
        <div style={{display:'grid', gridTemplateColumns:'minmax(0,1fr) minmax(0,1fr)', gap: 14}}>
          ...
          <div style={{display:'flex', flexDirection:'column', gap: 8, minWidth: 0}}>
            <IT.Toggle on={true} label="Private torrent (no DHT/PeX/LSD)"/>
            <IT.Toggle on={false} label="Optimize alignment"/>
            <IT.Toggle on={true} label="Calculate pieces automatically"/>
            <IT.Toggle on={true} label="Start seeding immediately"/>
            <IT.Toggle on={false} label="Ignore share-ratio limits"/>
      ...
      <div style={{
        ...
        <div onClick={e=>e.stopPropagation()} style={{
          ...
          <div style={{display:'flex', alignItems:'center', gap: 10, padding:'12px 16px', borderBottom: '1px solid var(--divider)'}}>
            <span style={{color:'var(--fg-2)'}}>{Icon.search({size:16})}</span>
          ...
          <div style={{padding:'8px 12px', borderTop:'1px solid var(--divider)', display:'flex', gap: 14, fontSize: 11, color:'var(--fg-3)'}}>
            ...
            <span><IT.Kbd>Ctrl</IT.Kbd><IT.Kbd>K</IT.Kbd> open/close</span>
            <span style={{flex:1}}/>
  ...
  function ConfirmDeleteDialog({ onClose }) {
    const [withData, setWithData] = React.useState(false);
    const names = ['Blender.4.2.LTS.Splash.Project.Files.zip', 'arXiv-astro-ph-2026-Q1-preprints.tar'];
    return (
      <div style={{
        ...
        <div onClick={e=>e.stopPropagation()} style={{
          width: 460, maxWidth:'92vw', background:'var(--bg-0)',
          border:'1px solid var(--border-1)', borderRadius:'var(--r-lg)',
          boxShadow:'var(--shadow-lg)', overflow:'hidden',
        }}>
          <div style={{padding:'18px 18px 14px', display:'flex', gap: 12}}>
            <div style={{
              width: 36, height: 36, flexShrink:0, borderRadius:'var(--r-md)',
              background:'rgba(251,86,91,0.12)', color:'var(--st-error)',
              display:'flex', alignItems:'center', justifyContent:'center',
            }}>{Icon.remove({size:18})}</div>
            <div style={{flex:1}}>
              <div style={{fontSize: 14, fontWeight: 600, marginBottom: 4}}>Remove {names.length} torrents?</div>
              <div style={{fontSize: 12.5, color:'var(--fg-2)', lineHeight: 1.5}}>They'll be removed from IronTide. By default the downloaded files stay on disk.</div>
              <div style={{marginTop: 10, maxHeight: 88, overflow:'auto', background:'var(--bg-1)', border:'1px solid var(--border-1)', borderRadius:'var(--r-md)', padding:'6px 10px'}}>
                {names.map((n,i) => <div key={i} className="mono" style={{fontSize: 11.5, color:'var(--fg-1)', padding:'2px 0', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis'}}>{n}</div>)}
              </div>
              <label style={{display:'flex', alignItems:'center', gap: 8, marginTop: 12, cursor:'pointer'}} onClick={()=>setWithData(!withData)}>
                <span style={{
                  width: 16, height: 16, borderRadius: 4, flexShrink:0,
                  border:'1px solid ' + (withData ? 'var(--st-error)' : 'var(--border-2)'),
                  background: withData ? 'var(--st-error)' : 'transparent',
                  color:'#fff', display:'inline-flex', alignItems:'center', justifyContent:'center',
                }}>{withData ? Icon.check({size:11}) : null}</span>
                <span style={{fontSize: 12.5, color: withData ? 'var(--st-error)' : 'var(--fg-1)'}}>Also delete the downloaded files from disk</span>
              </label>
            </div>
          </div>
          <div style={{height: 50, borderTop:'1px solid var(--border-1)', background:'var(--bg-1)', display:'flex', alignItems:'center', justifyContent:'flex-end', padding:'0 14px', gap: 8}}>
            <IT.Btn variant="ghost" onClick={onClose}>Cancel</IT.Btn>
            <IT.Btn variant={withData ? 'solid' : 'primary'} onClick={onClose}
              style={withData ? {background:'var(--st-error)', borderColor:'var(--st-error)', color:'#fff'} : {}}>
              {withData ? 'Remove + delete files' : 'Remove'}
            </IT.Btn>
          </div>
        </div>
      </div>
  ...
  window.Modals = { AddTorrentDialog, CreateTorrentDialog, CommandPalette, ConfirmDeleteDialog };
})();
