...
fn main() -> Result<(), error::GuiError> {
    ...
    main_window.set_app_version(slint::SharedString::from(APP_VERSION));
    {
        ...
        st.prefs.webui_bind =
            if settings.qbt_compat.bind_address != "127.0.0.1" || api_config.bind.is_none() {
    ...
    // 5b. M219: restore persisted window geometry + detail-pane tab.
    //     Gated on first-run completion so the wizard always launches at
    //     the OS-WM default size, not at a previous run's tiny window.
    if !first_run::is_first_run() {
        if let Some(win_cfg) = gui_config.window.as_ref() {
            window_state::apply_window_config(&main_window, win_cfg);
        }
        if let Some(tab) = gui_config.detail_active_tab.as_deref() {
            main_window.set_detail_active_tab(tab.into());
            tab.clone_into(&mut state.lock().detail_active_tab);
        }
    }
...

    {
        ...
        main_window.on_add_torrent_magnet_changed(move |uri| {
            ...
            if let Ok(magnet) = irontide::core::Magnet::parse(uri_str.trim()) {
                ...
                let suggested = bridge::suggest_category(&name, &[], &[]).unwrap_or_default();
    ...
    }
    {
        ...
        main_window.on_ip_filter_unban_peer(move |ip| {
            let cmd_tx = cb_state.lock().cmd_tx.clone();
            if let Some(tx) = cmd_tx {
                let _ = tx.send(app::GuiCommand::IpFilterUnbanPeer { ip: ip.to_string() });
            }
        });
    }
    {
        ...
        timer.start(
            ...
            move || {
                ...
                if let Some(win) = weak.upgrade() {
                    ...
                    let new_state =
                        tray::derive_tray_state(has_downloading, has_seeding, has_error);
    ...
    main_window.on_wizard_finish({
        ...
        move || {
            if let Some(win) = weak.upgrade() {
                ...
                let listen_port: u16 = win
                    .get_wizard_listen_port()
                    .to_string()
                    .parse()
                    .unwrap_or(6881);
    ...
    // 10. Save GUI config if any dirty field (columns / skin / sidebar).
    {
            ...
            || st.detail_tab_dirty
        {
            ...
            // M219: capture window geometry. If the Slint event loop has
            // already torn down the platform window before this save block
            // runs, `as_weak().upgrade()` returns None and we warn rather
            // than silently dropping the field — see plan for rationale.
            if let Some(win) = main_window.as_weak().upgrade() {
                gui.window = Some(window_state::to_window_config(&win));
            } else {
                tracing::warn!(
                    "main window unavailable at shutdown save — window geometry not persisted"
                );
            }
            // M219: persist the active detail-pane tab.
            gui.detail_active_tab = Some(st.detail_active_tab.clone());
