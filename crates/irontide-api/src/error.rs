...
impl From<irontide::session::Error> for ApiError {
    fn from(e: irontide::session::Error) -> Self {
        use irontide::session::Error;
...
mod tests {
    ...
    fn torrent_not_found_maps_to_404() {
        let session_err = irontide::session::Error::TorrentNotFound(irontide::core::Id20::ZERO);
    ...
    fn duplicate_torrent_maps_to_409() {
        let session_err = irontide::session::Error::DuplicateTorrent(irontide::core::Id20::ZERO);
        let api_err = ApiError::from(session_err);
    ...
    fn invalid_settings_maps_to_400() {
        let session_err = irontide::session::Error::InvalidSettings("bad value".to_owned());
        let api_err = ApiError::from(session_err);
    ...
    fn config_error_maps_to_400() {
        let session_err = irontide::session::Error::Config("bad config".to_owned());
        let api_err = ApiError::from(session_err);
        assert_eq!(api_err.status, StatusCode::BAD_REQUEST);
        assert_eq!(api_err.code, "INVALID_REQUEST");
    }
    ...
    fn metadata_not_ready_maps_to_404() {
        let session_err = irontide::session::Error::MetadataNotReady(irontide::core::Id20::ZERO);
        let api_err = ApiError::from(session_err);
        assert_eq!(api_err.status, StatusCode::NOT_FOUND);
    ...
    fn shutdown_maps_to_503() {
        let session_err = irontide::session::Error::Shutdown;
        let api_err = ApiError::from(session_err);
